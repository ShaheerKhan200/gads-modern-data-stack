...
ad_legacy_data as (
    ...
    *  -- Removed the extra comma before '*'
    ...
    where segments_date > current_date - interval '14 days' --segments_date > (select max(segments_date) from {{ this }}) --surrogate_key > (select max(surrogate_key) from {{ this }}) -- exists or not 
